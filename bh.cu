...
__global__ void BuildTreeKernel(float* posx, float* posy, int* child, int* _bottom, int* _maxDepth, float radius, int N, int node_num,
    ...
    rootIndex = node_num;//root co index nam cuoi cung = 2*N
    if(threadId == 0){
    ...
    while(i < N){
        if(newBody){
            ...
            lastIndex = rootIndex;//nốt cuối cùng đang được xét
            newBody = false;
            curRad = radius;

            local_depth = 1;
        }
        curIndex = child[CELL_NUM*lastIndex+path];//lấy về body id đang nằm ở node CEll_NUM*lastIndex+path
        
        // follow a path to leave node ..đi theo các cell node k chứa body đi xuống
        while(curIndex >= N){//nghĩa là node hiện tại k chứa body 
            lastIndex = curIndex;//thì node dc xét gần đây nhất là node curIndex
        ...
        if (curIndex != LOCK) {//LOCK=-2
            locked = CELL_NUM*lastIndex+path;
            // try locking the node
            if (curIndex == atomicCAS((int*)&child[locked], curIndex, LOCK)) { //compare xem child[locked] == curIndex thì khóa child[locked] = -2
                if (curIndex == NOTHING) {//NOTHING= -1
                    child[locked] = i; // insert body and release lock gán child[locked] = stt của điểm đang xét trong đồ thị
                    local_max_depth = max(local_depth, local_max_depth);
                } else {//nếu dã có 1 body trong node thì tạo 1 cell chứa oldnode và newnode
                    // malloc new cell nodes
                    int old_node = curIndex;
                    float old_node_x = posx[old_node];
                    float old_node_y = posy[old_node];
                    int cell = atomicSub((int*)_bottom, 1) - 1;//khởi tạo id cho cell , mỗi lần tạo - đi 1
                    printf("");
                    int new_cell = cell;
                    ...
                    do{
                        ...
                        posx[cell] = posx[lastIndex] - curRad*0.5 + (path&1)*curRad;//tinh tam cua cell 
                        ...
                        path = 0;
                        if(posx[cell] < x){//xem newnode nằm ở vùng nào của cell
                            path += 1;
                        }
                        if(posy[cell] < y){
                            path += 2;
                        ...
                        old_node_path = 0;
                        if(posx[cell] < old_node_x){//xem old node nằm ở vùng nào của cell
                            old_node_path += 1;
                        }
                        if(posy[cell] < old_node_y){
                            old_node_path += 2;
                        }
                        if(path != old_node_path){//nếu old node và new node nằm ở 2 vùng khác nhau của cell
                            child[cell*CELL_NUM+path] = i;
                            child[cell*CELL_NUM+old_node_path] = old_node;
                            break;
                        }else{
                            lastIndex = cell;
                            cell = atomicSub((unsigned int*)_bottom, 1) - 1;//bottom = 2*N
                        }
                    }while(true);
                    __threadfence();
...
__global__ void SummarizeTreeKernel(float* posx, float* posy, int* child, int* count, int* _bottom, int node_num, int N){
    ...
    while(node_id<=node_num){//nodenum=2*Nư
        ...
        if(missing == 0){
            posx[node_id] = sum_x/tmp_count; //tmp_count: số con của node
            posy[node_id] = sum_y/tmp_count;
            //FENCE
            __threadfence();
            count[node_id] = tmp_count;//số con của node_id
            node_id += step;
        }
    }
...
__global__ void SortKernel(int* startd, int *sort, int *child, int *count,int *_bottom, int node_num) {
...
__global__ void ForceCalculationKernel(float* posx, float* posy, int* child, int* count, int *sort, int *E, int *Idx,float *dispX, float *dispY, int node_num, int *_maxdepth, float radius)
{
...
void BH(float* hostx, float* hosty, Edge *E, int *Idx, int N, int M, float K, int timesteps){
    ...
    dim3 gridDim(min((N*2+BLOCK_SIZE)/BLOCK_SIZE, 32));
    float alpha = 4;
    float eps = 0.0025;
    float thr = W+H;
    float* posx;
    ...
    for (iter = 0; iter < timesteps; iter++) {
        ...
        rootx = (minx+maxx)*0.5;
        rooty = (miny+maxy)*0.5;
        cudaMemset(_bottom, node_num, 1);//bottom =node_num = 2*N
        cudaMemset(_maxDepth, 0, 1);//Gán maxDepth=0
        ...
        cudaDeviceSynchronize();
        batch_set(child, CELL_NUM*(node_num+1), -1);//khởi tạo danh sách node rỗng = -1
