...
void initial(float *posx, float *posy,int N){
    for(int i=0;i < N;i++){
        posx[i]= float(rand())/RAND_MAX-0.5;
        posy[i]= float(rand())/RAND_MAX-0.5;
    }
}
...
void force_directed(float *posx, float *posy, int *edges, int *Idx, int N, int M, float K, int Iteration, float thr){
    ...
    float *disp_x = new float[N]();//mảng các tọa độ x dc cập nhật
    float *disp_y = new float[N]();//mảng các tọa độ y dc cập nhật
    printf("N = %d, M = %d\n", N, M);//N là số đỉnh, M là số cạnh 
    ...
    for(int itr=0; itr<Iteration; ++itr){
        ...
        #pragma omp parallel for default(shared) schedule(dynamic)
        for(int v=0; v<N; ++v){//cập nhật vị trí
...
int main(int argc, char* argv[]) {
    ...
    //2 phan tu lien tiep trong mang edge là hoán vị của nhau
    Edge e;
    ...
    sort(E, E+2*M, cmp);// sắp xếp tập các cạnh theo id của đỉnh 1 rồi đến đỉnh 2
    for(int i=0; i<2*M; ++i) {
        Idx[E[i].idx1] += 1; // Idx[i] lúc này chứa số cặp cạnh có idx1 là đỉnh i
    }
    for(int i=1; i<N; ++i) {
        Idx[i] += Idx[i-1]; //End Index  Idx[] = [1,2,7]
    }
    //Load tọa độ 2 điểm trong E[i] vào 2 phần tử liên tiếp trong edges 
    for(int i=1; i<2*M; ++i) {
        edges[2*i] = E[i].idx1;
